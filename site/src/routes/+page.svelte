<script lang="ts">
  ...
  onMount(async () => {
    ...
    map = L.map('mapid', { attributionControl: false }).setView([31.2813, -98.7940], 5);
      ...
      if (numFireIncidents == 0) {
        ...
        for (var i = 0; i < fireIncidents.length; i++) {
          var marker = L.marker([fireIncidents[i]["latitude"], fireIncidents[i]["longitude"]], { icon: flameIcon }).addTo(map);
          marker.bindTooltip(fireIncidents[i]["fire_name"] + "<br>" + fireIncidents[i]["area"] + " acres burned<br>" + fireIncidents[i]["report_date"])
        }
      }
  });
</script>
...
<main>
  <div class="p-5">
    {#if numFireIncidents == 0}
     <h5 class="mb-2 text-xl font-bold text-gray-900 dark:text-gray-900">No, there are no large wildland fires as of { fetchDate }</h5>
    {:else if numFireIncidents == 1}
     <h5 class="mb-2 text-xl font-bold text-gray-900 dark:text-gray-900">Yes, there is { numFireIncidents } large wildland fire</h5>
    {:else}
      <h5 class="mb-2 text-xl font-bold text-gray-900 dark:text-gray-900">Yes, there are { numFireIncidents } large wildland fires</h5>
    {/if}
    {#if numFireIncidents > 0}
      <h3 class="mb-2 text-xl text-gray-900 dark:text-gray-900">{ formattedFireArea } acres are burning as of { fetchDate }</h3>
    {/if}
  </div>
  ...
  <div class="p-5">
    ...
    This app uses the most recent <a href="http://fsapps.nwcg.gov/afm/" class="underline">large fire incident data set</a> from the USDA Forest Service.<br />
